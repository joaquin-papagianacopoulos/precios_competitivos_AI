...
class PriceListProcessor:
    ...
    def find_column_index(self, df, possible_names):
        """Encuentra el indice de la columna basandose en nombres posibles"""
    ...
    def find_column_in_first_rows(self, df, possible_names, max_rows=8):
        ...
        util cuando los headers reales estan en filas posteriores
        """
        ...
        for row_idx in range(search_rows):
            ...
            for col_idx, cell_value in enumerate(row_values):
                if cell_value:  # Solo si la celda no esta vacia
                    for name in possible_names:
    ...
    def clean_price(self, price_str):
        ...
        # RECHAZO INMEDIATO: Si el string contiene letras (excepto simbolos de moneda), no es un precio
        ...
        # RECHAZO INMEDIATO: Si es solo un numero sin formato de precio (sin $, puntos, comas)
        # y es menor a 3 digitos, probablemente es cantidad, no precio
        ...
        price_str = re.sub(r'^\p{Sc}+\s*', '', price_str)

        # Remover caracteres no numericos excepto puntos y comas
        clean_str = re.sub(r'[^\p{L}\p{N}]', '', price_str)
        
        # Si despues de limpiar no queda nada o solo caracteres, rechazar
        if not clean_price or clean_price in ['.', ',', '.,', ',.']:
            return None
        
        # Si es un solo digito despues de limpiar, probablemente no es precio
        ...
        try:
            ...
            for sheet_name in excel_file.sheet_names:
                ...
                try:
                    ...
                    if df.empty:
                        print(f"⚠️ Hoja {sheet_name} esta vacia")
                        debug_info.append(f"Hoja {sheet_name}: vacia")
                        continue
                    ...
                    if product_col_idx is None:
                        print(f"❌ No se encontro columna de producto en {sheet_name}")
                        debug_info.append(f"Hoja {sheet_name}: No se encontro columna de producto")
                        continue
                        
                    if price_col_idx is None:
                        print(f"❌ No se encontro columna de precio en {sheet_name}")
                        ...
                        debug_info.append(f"Hoja {sheet_name}: No se encontro columna de precio. Columnas disponibles: {list(df_with_header.columns)}")
                    ...
                    # VALIDACIoN CRiTICA: Verificar que producto y precio son columnas diferentes
                    if product_col_idx == price_col_idx:
                        print(f"❌ ERROR CRiTICO: La misma columna ({product_col_idx}) se detecto para producto Y precio")
                        print(f"🔍 Esto indica que no se encontro una columna de precio valida")
                        print(f"📋 Columnas disponibles: {list(df_with_header.columns)}")
                    ...
                    if header_row > 0:
                        ...
                        # Ajustar los indices de columna ya que el DataFrame cambio
                    ...
                    for idx, row in df_processed.iterrows():
                        try:
                            ...
                            # DEBUG: Mostrar exactamente que esta leyendo
                            ...
                            # Solo procesar si hay un producto valido
                            if pd.notna(product) and str(product).strip():
                                ...
                                if products_in_sheet < 3:
                                    ...
                                    if price is None:
                                        print(f"   ❌ PRECIO RECHAZADO: '{price_raw}' no es un precio valido")
                                    else:
                    ...
                    print(f"✅ Productos validos en {sheet_name}: {products_in_sheet}")
                    if skipped_invalid_prices > 0:
                        print(f"⚠️ Precios invalidos omitidos: {skipped_invalid_prices}")
                    debug_info.append(f"Hoja {sheet_name}: {products_in_sheet} productos validos, {skipped_invalid_prices} precios invalidos omitidos")
            ...
            if len(all_products) == 0:
                print(f"⚠️ DIAGNoSTICO COMPLETO PARA {supplier_name}:")
            ...
            # Cerrar el archivo Excel explicitamente
...
def upload_file():
    ...
    if 'file' not in request.files:
        return jsonify({'success': False, 'message': 'No se selecciono archivo'}), 400
    ...
    try:
        ...
        return jsonify({
            ...
            'proveedor_status': proveedor_status,
            'db_status': db_status,
            'debug_info': debug_info[:5]  # un poquito de diagnostico
        }), 200
...
def generate_pdfs():
    ...
    if not business_data_from_db or not all([
        ...
        return jsonify({
            ...
            'message': 'Primero complete la informacion de su comercio'
        }), 400
    ...
    for supplier, items in suppliers.items():
        ...
        try:
            ...
            # Telefono del proveedor: BD -> fallback a SUPPLIER_PHONES
            phone = get_supplier_phone(supplier)
            phone_clean = _normalize_phone(phone)

            # Armar URL wa.me con o sin numero
            if phone_clean:
